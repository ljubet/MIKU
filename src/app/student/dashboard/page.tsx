...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const el = detailRefs.current[selectedJobId];
    if (el) {
      el.scrollIntoView({ behavior: "smooth", block: "start" });
    }
  }, [selectedJobId]);

  const renderDetailContent = (job: Job) => {
    const jobMatchScore = computeMatchScore(job.tags, currentStudent.skills);
    const jobMatchedSkills = job.tags.filter((t) =>
      currentStudent.skills.map((s) => s.toLowerCase()).includes(t.toLowerCase())
    );
    const jobMissingSkills = job.tags.filter(
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-gray-500 mt-0.5 flex items-center gap-1.5 flex-wrap">
                <Link
                  ...
                  className="hover:text-[#FF0078] transition-colors"
                >
                  {job.orgName}
                </Link>
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <span
                className={`text-sm font-bold flex items-center gap-1 ${
                  jobMatchScore >= 80 ? "text-emerald-600" : "text-[#FF0078]"
                }`}
              >
            ...
            <div className="w-full bg-gray-100 rounded-full h-1.5 mb-4">
              <div
                className={`h-1.5 rounded-full transition-all ${
                  jobMatchScore >= 80 ? "bg-emerald-500" : "bg-[#FF0078]/100"
                }`}
                style={{ width: `${jobMatchScore}%` }}
              />
            </div>
            <div className="space-y-2.5">
              {jobMatchedSkills.map((s) => (
                <div key={s} className="flex items-center justify-between">
                  <span className="text-sm text-gray-700">{s}</span>
                  <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                </div>
              ))}
              {jobMissingSkills.map((s) => (
                <div key={s} className="flex items-center justify-between">
    ...
    <div className="-mx-4 -my-4 md:-mx-8 md:-my-8 min-h-screen md:h-screen px-4 py-4 md:px-6 md:py-5 bg-gray-50 flex flex-col">
      ...
      <div className="flex flex-col md:flex-row gap-4 flex-1 md:min-h-0">
        ...
        <div className="w-full md:w-[38%] flex flex-col md:min-h-0">
          ...
          <div className="flex-1 space-y-2 pr-1 min-h-[60vh] md:min-h-0 md:overflow-y-auto md:max-h-none">
            {filtered.length === 0 ? (
              ...
              filtered.map((job) => {
                ...
                return (
                  <div key={job.id} className="space-y-3">
                    <div
                      onClick={() =>
                        setSelectedJobId((prev) => (prev === job.id ? "" : job.id))
                      }
                    ...
                    {isSelected && selectedJob ? (
                      <div
                        ref={(el) => {
                          detailRefs.current[job.id] = el;
                        }}
                        className="md:hidden bg-white border border-gray-200 rounded-2xl overflow-hidden flex flex-col min-h-[70vh]"
                      >
                        {renderDetailContent(selectedJob)}
                        ...
                        <div className="sticky bottom-0 z-10 bg-white/95 backdrop-blur border-t border-gray-100 px-4 py-3">
                          {isApplied ? (
                            <div className="flex items-center justify-between gap-3">
                              <button
                                ...
                                className="text-xs font-semibold text-gray-500 border border-gray-200 rounded-full px-3 py-1.5 hover:text-gray-700 hover:border-gray-300 transition-colors"
                              >
                                {t('btn_cancel')}
                              </button>
                            ...
                            <button
                              ...
                              className={`w-full text-white text-sm font-semibold px-6 py-3 rounded-xl transition-colors ${
                                limitReached
        ...
        {selectedJob ? (
          <div className="hidden md:flex w-full md:w-[62%] bg-white border border-gray-200 rounded-2xl flex-col min-h-[65vh] md:min-h-0">
            {renderDetailContent(selectedJob)}
            <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between shrink-0">
              ...
              {isApplied ? (
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => cancelApplication(selectedJob.id)}
                    className="text-xs font-semibold text-gray-500 border border-gray-200 rounded-full px-3 py-1.5 hover:text-gray-700 hover:border-gray-300 transition-colors"
                  >
                    {t('btn_cancel')}
                  </button>
